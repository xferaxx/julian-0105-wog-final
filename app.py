...
# a function to choose a level difficulty for the user
def level_difficulty():
    while True:
        try:
            level = int(input("please choose level difficulty between  1 - 5"))
            if 1 <= level <= 5:
                return level
            else:
...
# a function to check what is the user input to choose the game
...
# a function to start the first game ( memory game ).
def play1(level):
